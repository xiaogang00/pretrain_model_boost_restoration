...
class VideoBaseModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if train_opt['ft_tsa_only']:
                ...
                optim_params_name=[]
                for k, v in self.netG.named_parameters():
                    ##if v.requires_grad and ('m' in k.replace('module.','') or 'refine' in k.replace('module.','')) and not('transformer' in k):
                    if v.requires_grad:
                        optim_params.append(v)
                        optim_params_name.append(k)
                    else:
                        if self.rank <= 0:
                            logger.warning('Params [{:s}] will not optimize.'.format(k))
            
            print(optim_params_name)
    ...
    def optimize_parameters(self, step):
        ...
        #####################################################################################
        mean_feature=torch.FloatTensor([0.48145466, 0.4578275, 0.40821073]).view(1,3,1,1).to(mask.device)
        std_feature=torch.FloatTensor([0.26862954, 0.26130258, 0.27577711]).view(1,3,1,1).to(mask.device)
        
        real_low=self.var_L
        real_low=torch.clamp(real_low,min=0,max=1)
        real_low=(real_low-mean_feature)/std_feature
        image_features2 = self.clip_model.encode_image(real_low)
        image_features2n = image_features2/image_features2.norm(dim=-1, keepdim=True)
        
        clip_condition_list=[]
        clip_condition_list.append(image_features2n)
        
        self.fake_H, self.fake_h = self.netG(self.var_L, mask, clip_condition_list)
        
        l_pix = self.l_pix_w * self.cri_pix(self.fake_H, self.real_H)
        l_vgg = self.l_pix_w * self.cri_vgg(self.fake_H, self.real_H) * 0.1

        l_pix2 = self.l_pix_w * self.cri_pix(self.fake_h, self.real_H)
        l_vgg2 = self.l_pix_w * self.cri_vgg(self.fake_h, self.real_H) * 0.1
        
        l_final = l_pix + l_vgg + l_pix2 + l_vgg2
        l_final.backward()
        self.optimizer_G.step()
        self.log_dict['l_pix'] = l_pix.item()
        self.log_dict['l_vgg'] = l_vgg.item()
        self.log_dict['l_pix2'] = l_pix2.item()
        self.log_dict['l_vgg2'] = l_vgg2.item()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            clip_condition_list=[]
            clip_condition_list.append(image_features2n)
            self.fake_H, _ = self.netG(self.var_L, mask, clip_condition_list)
        self.netG.train()
